...
async function startServer() {
  ...
  // Endpoint racine & Health check pour le Backend API
  app.get("/", (_req, res) => {
    res.json({
      status: "online",
      service: "e-EnergieTEC MDMS & Smart Prepayment (STS) Backend API",
      version: "7.0.0",
      compliance: ["STS IEC 62055-41", "DLMS/COSEM IEC 62056", "ISO 27001", "CIM IEC 61968"],
      port: PORT,
  ...
  app.get("/api/health", (_req, res) => {
    res.json({
      status: "healthy",
      database: isEnterpriseMode ? "PostgreSQL / TimescaleDB" : "SQLite",
      timestamp: new Date().toISOString()
    });
  });

  // Handler 404 pour les routes API inconnues
  app.use("/api/*", (req, res) => {
    res.status(404).json({ error: `API route not found: ${req.method} ${req.originalUrl}` });
  });

  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[BACKEND API] e-EnergieTEC Server running on http://localhost:${PORT}`);
  });
  setInterval(() => {}, 100000);
}
